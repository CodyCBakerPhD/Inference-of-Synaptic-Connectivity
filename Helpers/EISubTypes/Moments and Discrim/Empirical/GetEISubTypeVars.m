...
function SubTypeMeans = GetEISubTypeStds(Mat,BiDirIds)
    ...
    if ( any(EE0ids(:)) )
        Mat_EE0 = Mat(EE0ids);
        SubTypeMeans.EE0 = var(Mat_EE0);
    end
    
    if ( any(EE1ids(:)) )
        Mat_EE1 = Mat(EE1ids);
        SubTypeMeans.EE1 = var(Mat_EE1);
    end
    
    if ( any(EE2ids(:)) )
        Mat_EE2 = Mat(EE2ids);
        SubTypeMeans.EE2 = var(Mat_EE2);
    end

    if ( any(II0ids(:)) )
        Mat_II0 = Mat(II0ids);
        SubTypeMeans.II0 = var(Mat_II0);
    end
    
    if ( any(II1ids(:)) )
        Mat_II1 = Mat(II1ids);
        SubTypeMeans.II1 = var(Mat_II1);
    end
    
    if ( any(II2ids(:)) )
        Mat_II2 = Mat(II2ids);
        SubTypeMeans.II2 = var(Mat_II2);
    end
    
    if ( any(EIIE0ids(:)) )
        Mat_EIIE0 = Mat(EIIE0ids);
        SubTypeMeans.EIIE0 = var(Mat_EIIE0);
    end
    
    if ( any(EIids(:)) )
        Mat_EI = Mat(EIids);
        SubTypeMeans.EI = var(Mat_EI);
    end
    
    if ( any(IEids(:)) )
        Mat_IE = Mat(IEids);
        SubTypeMeans.IE = var(Mat_IE);
    end

    if ( any(EIIEids(:)) )
        Mat_EIIE = Mat(EIIEids);
        SubTypeMeans.EIIE = var(Mat_EIIE);
    end
end
